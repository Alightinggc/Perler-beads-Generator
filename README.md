...
双击 **`拼豆图纸网页版.bat`**（或命令行 `python webui.py`），会自动打开浏览器，呈现一个现代化的网页界面：
...
> 说明：网页界面由 `webui.py`（本地 HTTP 服务）+ `web/index.html` 实现，**零额外依赖**（仅需 numpy + Pillow）。关闭服务 = 关闭运行 bat 弹出的那个命令行窗口（或按 Ctrl+C）。
