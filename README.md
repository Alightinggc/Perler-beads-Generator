...
- **[9] 图纸标题**、**[A] 过滤背景(过滤色+容差，类似PS取色去背)**、**[B] 底板尺寸(宽×高)**：手动输入
...
--bg-hex '#FFFFFF'  # 过滤色：把该颜色当背景（空格不填豆，类似PS取色去背）
--bg-tol 30         # 容差：该颜色多大范围内的颜色都算背景（0~255，越大删得越多）
--auto-bg           # 自动把图片四周主色识别为背景

> 背景过滤只在容差范围内生效，不会误删头发/衣服等非背景的浅色；被过滤的背景不参与颜色映射与抖动。
```
