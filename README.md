...
### 新电脑 / 发给别人用：先双击 `一键安装环境.bat`

对方电脑上如果没装 Python，直接双击任何 .bat 都会失败。**先把整个文件夹发给对方，让对方双击一次 `一键安装环境.bat`**，脚本会自动：

1. 检测 Python（没有则尝试用 winget 自动安装，失败会打开官网下载页引导手动安装）
2. 在本目录创建虚拟环境 `.venv`（**不污染对方电脑原有的 Python 环境**）
3. 安装全部依赖（numpy + Pillow；默认源失败会自动切换清华镜像重试）
4. 验证依赖，提示安装完成

装好后所有 .bat 入口都会**优先使用本目录 `.venv` 里的 Python**，直接拖图片就能用。

> 如果之前安装失败，先删除本目录的 `.venv` 文件夹，再重新双击 `一键安装环境.bat`。
> 脚本也会自动检测并重建不可用的 `.venv`（例如从别的电脑直接复制过来的）。
>
> 打包发给别人时，建议**排除 `.venv` 文件夹**（它是本机专用环境，别人装了也没用），
> 让对方拿到包后自己双击 `一键安装环境.bat` 即可。

### 备选方案：打包成免环境的 exe（PyInstaller）

如果对方完全不想装 Python，也可以在本机先打包出 4 个独立 exe（`packaging/build_exe.bat` 一键完成）：
...
这些 exe 已内置 Python + numpy + Pillow + 色板数据（`mard.json`、`web/index.html`），
**对方电脑什么都不用装**，双击或把图片拖到 exe 上即可使用。构建产物在 `dist/` 目录
（已加入 `.gitignore`，不随源码提交，按需发给别人即可）。
...
### 网页版界面（推荐）：`拼豆图纸网页版.bat`

双击 **`拼豆图纸网页版.bat`**（或命令行 `python webui.py`），会自动打开浏览器，呈现一个现代化的网页界面（不再依赖 cmd 菜单）：
...
> 说明：网页界面由 `webui.py`（本地 HTTP 服务）+ `web/index.html` 实现，完全复用 `main.py` 的转换逻辑，**零额外依赖**（仅需 numpy + Pillow）。关闭服务 = 关闭运行 bat 弹出的那个命令行窗口（或按 Ctrl+C）。
...
dot/
├── main.py                 # 命令行入口
├── interactive.py          # cmd 文本菜单（参数自选.bat 入口）
├── webui.py                # 网页版界面本地服务器（网页版.bat 入口）
├── web/index.html          # 网页版界面（HTML+CSS+JS）
├── 一键安装环境.bat          # 新电脑先运行它：装 Python + 建 .venv + 装依赖
├── 拼豆图纸精简版.bat        # 拖拽即转（懒人版）
├── 拼豆图纸参数自选.bat      # cmd 菜单版入口
├── 拼豆图纸网页版.bat        # 网页版入口
